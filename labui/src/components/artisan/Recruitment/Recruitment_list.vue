		...
		<el-dialog title="添加招聘信息" :visible.sync="addDialogVisble" width="70%" @close="addDialogClosed">
			...
			<el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="150px">
				...
				<el-form-item label="申请部门" prop="sectionId">
					<el-input v-if="false" disabled="disabled" v-model="addForm.sectionId"></el-input>
				...
				<el-form-item label="实验室" prop="labName">
					<el-input v-if="false" disabled="disabled" v-model="addForm.labId"></el-input>
			...
			<el-table :data="reviewtableData" border style="width: 100%">
				...
				<el-table-column fixed="right" label="操作">
					<template slot-scope="scope">
						...
						<div style="margin-bottom: 10px">
						<el-button @click="entry2(scope.row,2)" type="primary" icon="el-icon-edit">同意</el-button>
						</div>
	...
	export default {
		methods: {
			...
			mapTableToOptions() {
				...
				_this.options = _this.tableData2.map(item => ({
					value: item.labId,
					label: item.labName,
				}));
			},
			...
			review: function(row) {
				...
				this.$axios.get('http://localhost:8083/form/review/' + row.id).then(resp => {
					...
					_this.labId=row.labId;
					_this.sectionId=row.sectionId;
					console.log("各门id")
					console.log(_this.labId);
			...
			entry(row, num) {
				const _this = this
				this.$confirm('此操作拒绝录取该同学, 是否确定?', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					this.$axios.get('http://localhost:8083/form/interview/' +_this.sectionId+ '/'+row.planId + '/' + row.userId + '/' + num).then(
							function(resp) {
								row.id = row.planId
								_this.review(row);
			...
			submitForm(){
				...
				this.$refs['addFormRef'].validate((valid) => {
					if (valid) {
						this.addForm.labId = Number(this.addForm.labId);
						this.addForm.sectionId = Number(this.addForm.sectionId);
