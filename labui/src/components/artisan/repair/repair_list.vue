				...
				<el-table-column fixed="right" label="操作">
					<template slot-scope="scope">
						...
						<div style="margin-bottom: 10px">
							<el-button v-if="scope.row.status === 0" @click="submitEditForm(scope.row)" type="warning" >未维修</el-button>
							<el-button v-if="scope.row.status === 1"disabled type="success">已经维修</el-button>
						</div>
	...
	export default {
		methods: {
			...
			submitEditForm(row) {
				...
				console.log(row);
				this.$axios
						.post('http://localhost:8083/repair/update', row )
						.then(function(resp) {
